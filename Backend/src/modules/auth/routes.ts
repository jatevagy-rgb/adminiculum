...
router.get('/me', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const isAzure = (req as any).azureAdToken === true;
    const autoProvision = process.env.ENABLE_AZURE_AD_AUTO_PROVISION === 'true';
    const result = isAzure
      ? await authService.getMeByClaims(userId, email, role, autoProvision)
