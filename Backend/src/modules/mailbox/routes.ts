...
import { getTransactionalMailTransport } from './transactionalMail';
...
function safeError(res: Response, error: unknown) { const e = error instanceof MailboxServiceError ? error : null; return res.status(e?.status ?? 500).json({ code: e?.code ?? 'MAILBOX_OPERATION_FAILED', message: e?.message ?? 'Mailbox operation could not be completed.' }); }
...
async function oauthCallback(req: Request, res: Response, expected: MailboxProviderCode) { try { const state = verifyOAuthState(String(req.query.state || req.body?.state || '')); if (state.provider !== expected) throw new MailboxServiceError(400, 'MAILBOX_OAUTH_STATE_INVALID'); const mailbox = await ownedMailbox(state.connectionId, state.userId); const adapter = getMailboxProvider(expected); const redirectUri = expected === 'MICROSOFT_GRAPH' ? String(process.env.MICROSOFT_MAILBOX_REDIRECT_URI || '') : String(process.env.GOOGLE_MAILBOX_REDIRECT_URI || ''); const authorized = await adapter.exchangeAuthorizationCode({ code: String(req.query.code || req.body?.code || ''), redirectUri }); if (authorized.authorizedAddress && normalizeMailboxAddress(authorized.authorizedAddress) !== mailbox.mailboxAddress) throw new MailboxServiceError(403, 'MAILBOX_PROVIDER_IDENTITY_MISMATCH'); const secretReference = `mailbox-${crypto.randomUUID()}`; await getSecretStore().put(secretReference, authorized.secret); await prisma.communicationMailboxConnection.update({ where: { id: mailbox.id }, data: { secretReference, status: 'CONNECTED', readCapability: true, sendCapability: true, providerAccountId: authorized.providerAccountId ?? null, providerTenantId: authorized.providerTenantId ?? null } }); res.json({ status: 'CONNECTED' }); } catch (e) { safeError(res, e); } }
