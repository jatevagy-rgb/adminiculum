 ...
 * - TWO VISIBILITY LAYERS, NEVER COLLAPSED (P1 authorization repair). The
 *   reader takes an already-resolved `LegalSourceImpactAccessScope`:
 *     - CLIENT-LEVEL impact (requirement / control / applicability / client
 *       summary) is filtered by the caller's canonical CLIENT read scope;
 *     - DOCUMENT-LEVEL impact (DOCUMENT_REFERENCE_IMPACT) is filtered by the
 *       caller's canonical CASE read scope plus the canonical HR_CONFIDENTIAL
 *       boundary. A readable case of one client NEVER unlocks another case's
 *       document metadata of the same client. `clientId` is never used as a
 *       substitute for document visibility. Document totals are always computed
 *       AFTER this filtering.
 *   The reader consumes the canonical scope resolved by the route; it never
 *   derives its own document ACL.
...
/* ------------------------------------------------------------------ */
/*  Resolved access scope — resolved by the caller from the CANONICAL   */
/*  policy, never derived here.                                         */
/* ------------------------------------------------------------------ */

/**
 * Canonical, already-resolved visibility for ONE impact projection.
 *
 * Deliberately separates CLIENT-level from DOCUMENT-level visibility so that
 * client-scoped access can never be used as a substitute for document
 * visibility. Each field mirrors an existing canonical policy helper:
 *  - `readableClientIds` / `readableCaseIds`: `internalCaseScope` (ADMIN/PARTNER
 *    are unscoped → null; every other role is limited to its accessible cases,
 *    and to the clients that contain at least one such case — exactly the
 *    `assertClientReadAccess` rule).
 *  - `hrConfidentialReadAllowed`: the canonical document-classification
 *    boundary (ADMIN/PARTNER only), applied exactly as `company-workspace` does.
 *
 * `null` means "unscoped" and is reserved for the canonical privileged policy
 * (ADMIN/PARTNER) or a trusted system caller. An empty set means "no access".
 * Document impact is filtered by `readableCaseIds` + this HR boundary, NEVER by
 * `readableClientIds`.
 */
export interface LegalSourceImpactAccessScope {
  readonly readableClientIds: ReadonlySet<string> | null;
  readonly readableCaseIds: ReadonlySet<string> | null;
  readonly hrConfidentialReadAllowed: boolean;
}

/** Canonical privileged / trusted-system scope (ADMIN/PARTNER semantics). */
export const UNSCOPED_IMPACT_ACCESS: LegalSourceImpactAccessScope = {
  readableClientIds: null,
  readableCaseIds: null,
  hrConfidentialReadAllowed: true,
};

/** Fail-closed default: an omitted scope exposes nothing. */
const FAIL_CLOSED_IMPACT_ACCESS: LegalSourceImpactAccessScope = {
  readableClientIds: new Set<string>(),
  readableCaseIds: new Set<string>(),
...
/**
 * CLIENT-LEVEL filter — the canonical client read scope. Used only for the
 * client-bearing requirement / control / applicability rows and never for
 * document impact.
 */
function buildClientScopeWhere(
  scope: LegalSourceImpactAccessScope,
): { clientId?: { in: string[] } } {
  return scope.readableClientIds ? { clientId: { in: [...scope.readableClientIds] } } : {};
}

/** The Document-layer predicate the canonical policy applies to document reads. */
interface ImpactDocumentRelationWhere {
  caseId?: { in: string[] };
  securityClassification?: { not: 'HR_CONFIDENTIAL' };
}

/**
 * DOCUMENT-LEVEL filter — the canonical CASE read scope plus the canonical
 * HR_CONFIDENTIAL boundary, applied through the anchor's
 * DocumentVersion → Document relation. The actor's CLIENT scope is deliberately
 * NOT used here: an accessible case must never unlock another case's documents
 * of the same client, and HR_CONFIDENTIAL stays restricted to the privileged
 * roles exactly as in the canonical document policy.
 */
function buildDocumentRelationWhere(
  scope: LegalSourceImpactAccessScope,
): { documentVersion: { document: ImpactDocumentRelationWhere } } | Record<string, never> {
  const document: ImpactDocumentRelationWhere = {};
  if (scope.readableCaseIds) document.caseId = { in: [...scope.readableCaseIds] };
  const hrVisible = scope.readableCaseIds === null || scope.hrConfidentialReadAllowed;
  if (!hrVisible) document.securityClassification = { not: 'HR_CONFIDENTIAL' };
  return Object.keys(document).length > 0 ? { documentVersion: { document } } : {};
}
 ...
 *
 * `accessScope` is the caller's canonical read scope (see
 * `LegalSourceImpactAccessScope`). It is REQUIRED to be fail-closed when omitted:
 *  - `readableCaseIds` filters DOCUMENT_REFERENCE_IMPACT (document visibility),
 *    together with the canonical HR_CONFIDENTIAL boundary;
 *  - `readableClientIds` filters the client-level requirement / control /
 *    applicability rows.
 * The legal-source / requirement / control STRUCTURE is not client-scoped and is
 * always reported; only the client-bearing impact rows are filtered, and every
 * total is computed after that filtering.
 */
export async function buildLegalSourceImpactForVersion(
  legalSourceVersionId: string,
  prisma: PrismaClient = defaultPrisma,
  accessScope: LegalSourceImpactAccessScope = FAIL_CLOSED_IMPACT_ACCESS,
  generatedAt: string = new Date().toISOString(),
): Promise<LegalSourceImpactProjection | null> {
  const clientScopeWhere = buildClientScopeWhere(accessScope);
  ...
  const documentReferences = (await prisma.complianceDocumentClauseAnchor.findMany({
    where: {
      ...
      ...buildDocumentRelationWhere(accessScope),
    },
 ...
 *
 * `accessScope` behaves exactly as in `buildLegalSourceImpactForVersion`: the
 * document impact is filtered by the canonical CASE scope + HR_CONFIDENTIAL
 * boundary, never by the client scope alone.
 *
