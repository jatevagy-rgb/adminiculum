...
const requireClientAccess = async (req: Request, res: Response, next: Function) => {
  try {
    ...
    (req as any).clientId = userId; // In real implementation, link user to client
    next();
  } catch (error) {
...
router.get('/summary/:clientId', async (req: Request, res: Response) => {
  try {
    ...
    const allTimeEntries = await prisma.timeEntry.findMany({
      where: {
        matter: { clientId: clientId as string }
      },
      include: {
        department: {
          select: { id: true, name: true }
    ...
    // Department breakdown
    const deptStats: Record<string, { name: string; minutes: number; matters: number }> = {};
...
router.get('/departments/:departmentId/matters', async (req: Request, res: Response) => {
  try {
    ...
    const result = matters.map((matter: any) => {
      const totalMinutes = matter.timeEntries.reduce((sum, e) => sum + e.minutes, 0);
      const lastEntry = matter.timeEntries.sort((a, b) => 
...
router.get('/matters/:matterId', async (req: Request, res: Response) => {
  try {
...
router.get('/matters/:matterId/time-log', async (req: Request, res: Response) => {
  try {
    ...
    const entries = await prisma.timeEntry.findMany({
      where: { matterId: matterId as string },
      include: {
        user: {
          select: { name: true }
        },
    ...
    const total = await prisma.timeEntry.count({ where: { matterId: matterId as string } });
  ...
  try {
    ...
    const exportData = {
      ...
      departments: departments.map((dept: any) => {
        const deptMinutes = dept.matters.reduce((sum, m) => 
          sum + m.timeEntries.reduce((s, e) => s + e.minutes, 0), 0
        );
        const activeMatters = dept.matters.filter(m => m.status === 'OPEN').length;

        return {
          name: dept.name,
          matters: dept.matters.map((m: any) => ({
            title: m.title,
            type: m.matterType,
            status: m.status,
            openedAt: m.openedAt,
