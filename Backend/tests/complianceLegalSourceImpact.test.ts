...
describe('C4C projection boundary', () => {
  ...
  it('exposes the impact projection through an internal-only route with requireInternal', () => {
    ...
    const handler = routesSource.slice(routeIndex, routeIndex + 2200);
    ...
    // The impact graph is cross-client internal, but it is scoped through the
    // canonical scope resolver; no per-client assertClientReadAccess is faked.
    expect(handler).not.toContain('assertClientReadAccess');
  });

  it('P1: resolves DOCUMENT visibility from the canonical CASE scope, never from clientId', () => {
    const routesSource = readFileSync(
      join(__dirname, '..', 'src', 'modules', 'compliance-doc-intelligence', 'routes.ts'),
      'utf8',
    );
    const scopeIndex = routesSource.indexOf('async function resolveImpactAccessScope');
    expect(scopeIndex).toBeGreaterThan(-1);
    const resolver = routesSource.slice(scopeIndex, scopeIndex + 1800);
    // Canonical reuse only: case scope + HR boundary + client read rule.
    expect(resolver).toContain('internalCaseScope');
    expect(resolver).toContain('hrConfidentialReadAllowed');
    expect(resolver).toContain('readableCaseIds');
    expect(resolver).toContain('readableClientIds');

    const readerSource = readFileSync(
      join(__dirname, '..', 'src', 'modules', 'compliance', 'legalSourceImpact.ts'),
      'utf8',
    );
    const documentWhere = readerSource.slice(
      readerSource.indexOf('function buildDocumentRelationWhere'),
      readerSource.indexOf('async function readClientNames'),
    );
    // Document visibility = case scope + HR_CONFIDENTIAL boundary.
    expect(documentWhere).toContain('document.caseId');
    expect(documentWhere).toContain('securityClassification');
    expect(documentWhere).toContain("not: 'HR_CONFIDENTIAL'");
    // The collapsed clientId-as-document-scope form is gone for good, and both
    // readers (registry version + canonical reference) use the case+HR filter.
    expect(readerSource).not.toContain('document: { clientId: { in:');
    expect(readerSource.match(/buildDocumentRelationWhere\(accessScope\)/g)?.length).toBe(2);
  });
