 ...
 *
 * and proves the two hard guards:
...
describeWithDatabase('C4C legal-source impact (PostgreSQL)', () => {
  ...
  const createDocument = async (id: string, clientId: string, caseId: string, name: string) =>
    db.document.create({
      data: {
        id,
        clientId,
  ...
  const addAnchor = async (
    ...
    overrides: { clauseRef?: string; relationType?: string; anchorKey?: string | null; legalSourceVersionId?: string | null },
  ) =>
  ...
  afterAll(async () => {
    ...
    await db.requirementApplicability.deleteMany({ where: { id: { in: [applicabilityOneId, applicabilityTwoId] } } });
    await db.applicabilityRuleVersion.deleteMany({ where: { id: ruleOneId } });
    await db.clientControl.deleteMany({ where: { clientId: { in: [clientAId, clientBId] } } });
    await db.requirementControlMap.deleteMany({ where: { requirementVersionId: { in: [requirementOneVersionId, requirementTwoVersionId] } } });
    await db.controlDefinition.deleteMany({ where: { id: { in: [controlOneId, controlTwoId] } } });
    await db.requirementCitation.deleteMany({ where: { requirementVersionId: { in: [requirementOneVersionId, requirementTwoVersionId] } } });
    await db.requirementVersion.deleteMany({ where: { id: { in: [requirementOneVersionId, requirementTwoVersionId] } } });
    await db.requirement.deleteMany({ where: { id: { in: [requirementOneId, requirementTwoId] } } });
    await db.legalSourceVersion.deleteMany({ where: { id: { in: [subjectVersionId, otherVersionId] } } });
    await db.legalSource.deleteMany({ where: { id: { in: [subjectSourceId, otherSourceId] } } });
  ...
  it('A. an exact registry anchor finds the correct client document; a wrong subject finds none of it', async () => {
    const subject = await buildLegalSourceImpactForVersion(subjectVersionId, db, null, GENERATED_AT);
    ...
    const other = await buildLegalSourceImpactForVersion(otherVersionId, db, null, GENERATED_AT);
    ...
    expect(await buildLegalSourceImpactForVersion(`c4c-missing-${suffix}`, db, null, GENERATED_AT)).toBeNull();
  });
  ...
  it('E. client impact keeps the three kinds separate and canonical references stay document-only', async () => {
    ...
    const reference = await buildDocumentReferenceImpactForCanonicalReference(MAIN_REFERENCE, db, null, GENERATED_AT);
    ...
    const wrong = await buildDocumentReferenceImpactForCanonicalReference(WRONG_REFERENCE, db, null, GENERATED_AT);
    ...
    expect(await buildDocumentReferenceImpactForCanonicalReference('https://example.com/foo', db, null, GENERATED_AT)).toBeNull();
  });
  ...
  it('F. reading the impact performs no write and creates no finding, proposal, task or legal version', async () => {
    ...
    await buildLegalSourceImpactForVersion(subjectVersionId, db, null, GENERATED_AT);
    await buildLegalSourceImpactForVersion(otherVersionId, db, null, GENERATED_AT);
    await buildDocumentReferenceImpactForCanonicalReference(MAIN_REFERENCE, db, null, GENERATED_AT);
    await buildLegalSourceImpactForVersion(subjectVersionId, db, null, GENERATED_AT);
  ...
  it('G. the external watcher manifest carries no client/document identity while the internal impact map does', async () => {
    ...
    const impact = await buildLegalSourceImpactForVersion(subjectVersionId, db, null, GENERATED_AT);
  ...
  it('H. respects the internal client read scope supplied by the caller', async () => {
    const scopedToA = await buildLegalSourceImpactForVersion(subjectVersionId, db, new Set([clientAId]), GENERATED_AT);
    ...
    const scopedToB = await buildLegalSourceImpactForVersion(subjectVersionId, db, new Set([clientBId]), GENERATED_AT);
    ...
    const scopedToNone = await buildLegalSourceImpactForVersion(subjectVersionId, db, new Set<string>(), GENERATED_AT);
