...
//
// Proves the client-scoped published-content projection is truthful:
//   A/B active member count counts only ACTIVE workspace memberships
//   C   published-content count excludes drafts / revoked / unpublished rows
//   D   exact-version document publication semantics are unchanged (read-only)
//   E   cross-client memberships never appear
//   F   cross-client publications never appear
//   M   no internal unpublished Document leaks
...
describeWithDatabase('Portal Center published-content projection (PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    ...
    await db.$connect();
    await db.user.createMany({ data: [{ id: ids.admin, email: 'portal-center-admin@example.invalid', name: 'Portal Center Admin', role: 'ADMIN', status: 'ACTIVE', isActive: true, skills: [] }] });
    ...
    await db.case.createMany({ data: [
      { id: ids.case, caseNumber: 'PCC-001', title: 'Portal center case', caseType: 'CONTRACT_REVIEW', clientId: ids.client, createdById: ids.admin, assignedLawyerId: ids.admin },
      { id: ids.otherCase, caseNumber: 'PCC-002', title: 'Other portal center case', caseType: 'CONTRACT_REVIEW', clientId: ids.otherClient, createdById: ids.admin, assignedLawyerId: ids.admin },
    ] });
    ...
    const workspace = await createWorkspace(actor, { clientId: ids.client, name: 'Portal Center Workspace', mode: 'ORGANIZATION', communicationMode: 'PORTAL_PRIMARY', connectedSystemState: 'NOT_CONFIGURED' }, db);
    workspaceId = workspace.id;
    const otherWorkspace = await createWorkspace(actor, { clientId: ids.otherClient, name: 'Other Workspace', mode: 'INDIVIDUAL', communicationMode: 'PORTAL_PRIMARY', connectedSystemState: 'NOT_CONFIGURED' }, db);
    otherWorkspaceId = otherWorkspace.id;

    const activeInvite = await inviteWorkspaceMember(actor, workspaceId, { email: 'portal-active@example.invalid', role: 'MEMBER' }, db);
    activeMembershipId = String(activeInvite.membershipId);
    const suspendedInvite = await inviteWorkspaceMember(actor, workspaceId, { email: 'portal-suspended@example.invalid', role: 'MEMBER' }, db);
    suspendedMembershipId = String(suspendedInvite.membershipId);
    const otherInvite = await inviteWorkspaceMember(actor, otherWorkspaceId, { email: 'portal-other@example.invalid', role: 'MEMBER' }, db);
    ...
    await transitionWorkspaceMembership(actor, activeMembershipId, 'approve', activeRow.revision, db);
    const suspendedRow = await db.clientPortalWorkspaceMembership.findUniqueOrThrow({ where: { id: suspendedMembershipId } });
    await transitionWorkspaceMembership(actor, suspendedMembershipId, 'approve', suspendedRow.revision, db);
    const suspendedApproved = await db.clientPortalWorkspaceMembership.findUniqueOrThrow({ where: { id: suspendedMembershipId } });
    await transitionWorkspaceMembership(actor, suspendedMembershipId, 'suspend', suspendedApproved.revision, db);
    const otherRow = await db.clientPortalWorkspaceMembership.findUniqueOrThrow({ where: { id: String(otherInvite.membershipId) } });
    await transitionWorkspaceMembership(actor, otherRow.id, 'approve', otherRow.revision, db);
  ...
  it('A/B. active member count counts only ACTIVE workspace memberships, excluding suspended', async () => {
    const { items } = await listAdminWorkspaces(actor, ids.client, db);
    ...
    expect(workspace!.memberships.filter((membership) => membership.status === 'ACTIVE')).toHaveLength(1);
    // canonical "invited by" resolution is additive and does not break the read model
    expect(workspace!.memberships.every((membership) => 'invitedByName' in membership)).toBe(true);
  });

  it('E. cross-client memberships never appear in the client-scoped read model', async () => {
    const { items } = await listAdminWorkspaces(actor, ids.client, db);
  ...
  it('C/M. published-content count excludes drafts, revoked and unpublished rows', async () => {
    const result = await getClientPublishedContent(actor, ids.client, db);
    expect(result.counts).toEqual({ matters: 1, documents: 1, actionRequests: 1, updates: 1, total: 4 });
    const idsSeen = result.items.map((item: any) => item.id);
    expect(idsSeen).toContain(ids.matterPublished);
    expect(idsSeen).toContain(ids.documentPublished);
    expect(idsSeen).toContain(ids.actionPublished);
    expect(idsSeen).toContain(ids.updatePublished);
    expect(idsSeen).not.toContain(ids.matterDraft);
    expect(idsSeen).not.toContain(ids.documentRevoked);
    expect(idsSeen).not.toContain(ids.actionDraft);
  });

  it('F. cross-client publications never appear', async () => {
    const result = await getClientPublishedContent(actor, ids.client, db);
    ...
    const other = await getClientPublishedContent(actor, ids.otherClient, db);
    expect(other.counts.total).toBe(1);
    expect(other.items[0].id).toBe(ids.otherMatterPublished);
  });

  it('D. exact-version document publication semantics are unchanged by the projection', async () => {
    const before = await db.clientDocumentPublication.findUniqueOrThrow({ where: { id: ids.documentPublished } });
    await getClientPublishedContent(actor, ids.client, db);
  ...
  it('rejects a non-internal actor and a client that does not exist', async () => {
    ...
    await expect(getClientPublishedContent(actor, 'a2000000-0000-4000-8000-0000000000ff', db)).rejects.toMatchObject({ code: 'CLIENT_NOT_FOUND' });
  });
});
