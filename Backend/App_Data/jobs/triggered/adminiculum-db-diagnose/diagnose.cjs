...
(async () => {
  ...
  let folderMigrations = [];
  try { const root2 = process.env.MIGRATION_WEBJOB_ROOT || path2.resolve(__dirname, '../../../..'); folderMigrations = fs2.readdirSync(path2.join(root2,'prisma','migrations')).filter(n=>/^2026/.test(n)); } catch(e){}
  ...
  process.stdout.write(JSON.stringify({ READONLY_DIAGNOSTIC:true, unfinishedCount: unfinished.rowCount, unfinished: map(unfinished.rows), targetFailedMigration: target.rows[0]?{...target.rows[0], logs:sanitize(target.rows[0].logs)}:null, triggerMigration: trig.rows[0]||null, objects: objs.rows[0], deployedMigrationFolders: { count: folderMigrations.length, hasStale_20260515190000: folderMigrations.includes('20260515190000_add_lawyer_handoff_package'), hasTrigger_20260802200000: folderMigrations.includes('20260802200000_client_identity_grant_trigger_fix') } })+"\n");
  await client.end();
