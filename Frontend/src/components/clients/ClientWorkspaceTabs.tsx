    ...
    <nav aria-label="Ügyfél munkaterület" className="border-b border-[var(--adm-border)]">
      <div className="flex min-w-0 flex-wrap items-center gap-1" role="tablist">
        ...
        <details className="relative ml-auto">
          <summary className="cursor-pointer list-none rounded px-3 py-2 text-xs font-semibold text-[var(--adm-text-muted)] hover:bg-[var(--adm-surface)]">
            ••• Haladó
          </summary>
          <div className="absolute right-0 z-20 mt-1 w-52 rounded border border-[var(--adm-border)] bg-white p-2 shadow-lg">
            <Link className="block rounded px-3 py-2 text-xs hover:bg-[var(--adm-surface)]" href={`/clients/${encodeURIComponent(clientId)}/vallalati-mukodes`}>Működés és megfelelés</Link>
            <Link className="block rounded px-3 py-2 text-xs hover:bg-[var(--adm-surface)]" href={`/clients/${encodeURIComponent(clientId)}/workgroups`}>Munkacsoportok</Link>
            <Link className="block rounded px-3 py-2 text-xs hover:bg-[var(--adm-surface)]" href={`/clients/${encodeURIComponent(clientId)}#house-style`}>Dokumentumstílus</Link>
          </div>
        </details>
      </div>
    </nav>
