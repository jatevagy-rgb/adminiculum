...
export function OrganizationEditor({ clientId, groups, persons, workspaces, action, onManagePermissionChanged, onSaved }: {
  ...
  const derivedPortal = person ? derivePortalMembership(person, workspaces) : null;
  const canRevokePortal = Boolean(derivedPortal?.membership && !["REVOKED", "EXPIRED"].includes(derivedPortal.membership.status));
