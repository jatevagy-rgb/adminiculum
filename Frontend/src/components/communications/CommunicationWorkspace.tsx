...
import { runOutlookSync, getOutlookStatus, type OutlookSyncSummary, type OutlookStatus } from "@/lib/api";
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`relative grid w-full gap-2 px-3 py-3 pl-5 text-left md:grid-cols-[1.05fr_1.45fr_0.9fr_0.75fr_0.6fr] ${selected?.id === item.id ? "bg-[var(--adm-sand-100)]/55" : "hover:bg-[var(--adm-surface)]"}`}>
                      ...
                      <span className="flex flex-wrap items-start gap-1"><StatusChip>{signal.direction === "incoming" ? "Bejövő" : "Kimenő"}</StatusChip><StatusChip>{classifyAudience(item) === "external" ? "Külső" : "Belső"}</StatusChip>{item.sourceTaskCount > 0 ? <StatusChip>{item.sourceTaskCount} feladat</StatusChip> : null}</span>
