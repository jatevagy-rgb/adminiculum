...
const roleLabels: Record<PortalWorkspaceSummary['membershipRole'], string> = {
  MEMBER: 'Ügyfél',
  REPRESENTATIVE: 'Kapcsolattartó',
  APPROVER: 'Szervezeti kapcsolattartó',
};
...
export function PortalWorkspaceSelector({ workspaces, onSelect }: { workspaces: PortalWorkspaceSummary[]; onSelect: (publicReference: string) => void }) {
  return (
    <section className="mx-auto max-w-5xl px-4 py-10 sm:px-6" data-testid="portal-workspace-selector">
      <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#b85f4b]">Ügyfélfelület választása</p>
      <h1 className="mt-3 font-serif text-4xl font-semibold">Melyik ügyfélfelületet szeretné megnyitni?</h1>
      <p className="mt-3 text-stone-700">Csak az aktív, az iroda által jóváhagyott ügyfélfelületei jelennek meg.</p>
      <div className="mt-8 grid gap-4 md:grid-cols-2">
        {workspaces.map((workspace) => (
          <article key={workspace.publicReference} className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
            ...
            <h2 className="mt-2 text-2xl font-semibold">{workspace.clientDisplayName} — {modeLabels[workspace.mode]}</h2>
