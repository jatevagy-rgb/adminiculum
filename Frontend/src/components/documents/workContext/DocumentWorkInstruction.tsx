...
export function DocumentWorkInstruction({
  ...
  if (!view.hasWorkInstruction) {
    return (
      <div data-testid="dwh-instruction-empty" className="flex min-w-0 flex-wrap items-center justify-between gap-2 rounded-md bg-[var(--adm-ivory-100)] px-2.5 py-1.5">
        <p className="min-w-0 text-[12px] text-[var(--adm-text-muted)]">Még nincs munkautasítás beállítva.</p>
        {canEdit && onEdit ? (
          <button
            type="button"
  ...
  return (
    <div data-testid="dwh-instruction" className={`rounded-md px-2.5 py-1.5 ${a.soft}`}>
      ...
      <p className="mt-0.5 whitespace-pre-line break-words text-[12.5px] leading-5 text-[var(--adm-text)]">{view.workInstruction}</p>
