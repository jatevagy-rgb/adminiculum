    ...
    <header className={`${isSignal ? "bg-[#0F172A] border-[#1F2937]" : "bg-[#F6F2E8] border-[#DDD7CA]"} border-b px-8 py-6`}>
      <div className="flex flex-wrap items-center gap-5">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Link
            href="/notifications"
            className={`relative h-10 w-10 border grid place-items-center ${isSignal ? "border-[#334155] bg-[#111827] text-[#CBD5E1]" : "border-[#DDD7CA] bg-white text-[#4F5A50]"}`}
            title="Értesítések"
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            ...
            {unreadNotifications > 0 ? (
              <span className={`absolute -right-1 -top-1 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${isSignal ? "bg-[#22D3EE] text-[#0B1220]" : "bg-[#C9A227] text-[#2D2A26]"}`}>
