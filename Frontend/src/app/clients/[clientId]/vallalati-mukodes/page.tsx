        ...
        <div className="adm-board-container space-y-5">
          ...
          {client ? (
            <>
              {organizationMode ? (
                <>
                  <ClientWorkspaceTabs clientId={client.id} active="company-operations" organizationMode={organizationMode} />
                  <ClientCompanyWorkspace clientId={client.id} clientName={client.name} />
                </>
              ) : (
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Ez a vállalati működés felület csak szervezeti ügyfélmódban érhető el.</div>
              )}
            </>
