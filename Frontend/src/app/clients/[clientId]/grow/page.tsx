        ...
        <div className="adm-board-container space-y-5">
          ...
          {client ? (
            <>
              {organizationMode ? (
                <>
                  <ClientWorkspaceTabs clientId={client.id} active="grow" organizationMode={organizationMode} />
                  <GrowJourney clientId={client.id} clientName={client.name} />
                </>
              ) : (
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">A Grow felület csak szervezeti ügyfélmódban érhető el.</div>
              )}
            </>
