...
import { isClosedCase, matchesCaseScope, casePriorityLabel, caseStatusLabel, caseDeadline, attentionForCase, matchesOperationalFilter, nextActionLabel, loadCaseAttentionPages } from '../src/lib/casesOperational';
...
test('deadline is localized and historical closed deadlines do not demand action', () => {
  ...
  assert.equal(caseDeadline(item).label, new Date(item.deadline).toLocaleDateString());
  assert.ok(caseDeadline(item).overdue);
  assert.ok(!caseDeadline({ ...item, status: 'FINAL' }).overdue);
  assert.equal(caseDeadline({ deadline: null, status: 'DRAFT' }).label, 'Nincs határidő');
});
