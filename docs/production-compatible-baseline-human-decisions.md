...
Final classification target: `cases_client_role_semantics_decision_documented_no_db_change_no_runtime_change`
...
- Case client role (`KEEP-BUT-HARDEN candidate`; semantics decided as internal matter-party metadata).
- Comparison snapshot.
- Workspace text.
...
| Case client role | `KEEP-BUT-HARDEN candidate` | CASES-CLIENT-ROLE-SEMANTICS-DECISION-1 created `docs/cases-client-role-semantics-decision.md` and selected Option A: internal matter-party metadata. `cases.clientRole` means an internal legal workflow classification of the represented client or relevant party role in the matter, useful for lawyer workflow, document-generation context, and anonymization/redaction targeting. It is not a Client Portal display field, tenant boundary, authorization primitive, external label, or enum contract. | Required before narrow internal `KEEP`: `CASES-CLIENT-ROLE-INTERNAL-HARDEN-1` to prove/add case-level authorization for generic case reads and manager/assigned-lawyer/privileged-role policy for writes, with targeted tests. Do not treat as Client Portal, external visibility, production apply, CP-SCHEMA-1, schema migration, or DB migration replay. |
...
- Whether client identity fields should move beyond `KEEP-BUT-HARDEN candidate` after dedicated authorization, DTO, and privacy decisions; and whether `cases.clientRole` should move beyond `KEEP-BUT-HARDEN candidate` after internal hardening now that semantics are internal matter-party metadata. `clients.color` is separately decided as narrow internal `KEEP` only.
