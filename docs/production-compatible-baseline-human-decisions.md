...
Final classification target: `case_collaborators_authz_audited_no_db_change_no_runtime_change`
...
| Case collaborators | `KEEP-BUT-HARDEN candidate` | CASE-COLLABORATORS-AUTHZ-AUDIT-1 created `docs/case-collaborators-authz-audit.md`. Production metadata compare found `case_collaborators` present-compatible and the repo has active internal usage, but generic collaborator read/write/delete routes appear to rely on general authentication only. Case-level authorization, manage-collaborator permission, delete path consistency, OpenAPI metadata posture, and cross-case tests remain required before any narrow internal `KEEP`. | Run `CASE-COLLABORATORS-HARDEN-1`; do not expose to Client Portal; do not treat present-compatible metadata as production apply authorization. |
...
PRESENT-COMPATIBLE-KEEP-CANDIDATES-AUDIT-1 created `docs/present-compatible-keep-candidates-audit.md`. Present-compatible metadata does not automatically promote items to `KEEP`: the audit classified `clients.color` as an internal `KEEP candidate`, `case_collaborators`, `workload_records`, client identity fields, and `cases.clientRole` as `KEEP-BUT-HARDEN candidate`, and `documents.workspaceText` as `SECURITY/PRIVACY BLOCKED`. CLIENTS-COLOR-INTERNAL-KEEP-DECISION-1 moves only `clients.color` from candidate to narrow internal `KEEP`. CASE-COLLABORATORS-AUTHZ-AUDIT-1 keeps `case_collaborators` in the `KEEP-BUT-HARDEN candidate` lane and records required authorization hardening before any internal `KEEP`. Production apply and CP-SCHEMA-1 remain blocked, and partial schema drift remains `QUARANTINE` unless separately decided.
