...
Final classification target: `workload_records_exposure_audited_no_db_change_no_runtime_change`
...
| Workload tracking / `workload_records` | `KEEP-BUT-HARDEN candidate` | WORKLOAD-RECORDS-INTERNAL-EXPOSURE-AUDIT-1 created `docs/workload-records-exposure-audit.md`. Production metadata compare found `workload_records` present-compatible and the repo has active internal workgroup/workload usage. Inspected workload/workgroup routes are authenticated, and public OpenAPI metadata quarantines workgroup/workload paths, but route code does not show role, current-user, client ownership, workgroup membership, case-level, or task-level authorization checks before workload read/write/summary operations. Client Portal remains disabled/quarantined. | Do not promote to `KEEP` until `WORKLOAD-RECORDS-HARDEN-1` adds or proves internal role/user/client/workgroup scoping and targeted tests. Do not treat as Client Portal, external visibility, CP-SCHEMA-1, production apply, or DB migration replay. |
