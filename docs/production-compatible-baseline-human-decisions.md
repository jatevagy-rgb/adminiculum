...
Final classification target: `workload_records_hardening_closeout_documented_no_db_change_no_runtime_change`
...
| Workload tracking / `workload_records` | `hardened internal KEEP candidate` | WORKLOAD-RECORDS-INTERNAL-EXPOSURE-AUDIT-1 created `docs/workload-records-exposure-audit.md`. Production metadata compare found `workload_records` present-compatible and the repo has active internal workgroup/workload usage. WORKLOAD-RECORDS-HARDEN-1 (`f6836d7`) added conservative `ADMIN` / `PARTNER` authorization for all current workgroup/workload read/write routes. Ordinary authenticated users receive `403 WORKLOAD_ACCESS_FORBIDDEN` before workload DB access. Targeted `workloadRecordsAuthz` tests passed, and full backend validation passed 14 suites / 152 tests. Public OpenAPI metadata still quarantines workgroup/workload paths. Client Portal remains disabled/quarantined. | Required before full narrow internal `KEEP`: explicit human decision, confirm no unreviewed workload/workgroup routes remain, confirm no external/client-portal exposure, preserve targeted tests, and keep production apply / CP-SCHEMA-1 blocked. Do not treat as Client Portal, external visibility, production apply, self-scoped lawyer/team access, or DB migration replay. Future non-admin scoped access requires a separate membership/ownership model and tests. |
