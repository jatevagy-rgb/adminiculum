...
Final classification target: `client_identity_role_fields_audited_no_db_change_no_runtime_change`
...
- Case client role (`KEEP-BUT-HARDEN candidate` and `NEEDS PRODUCT DECISION` after audit).
- Comparison snapshot.
- Workspace text.
...
| Case client role | `KEEP-BUT-HARDEN candidate` + `NEEDS PRODUCT DECISION` | CLIENT-IDENTITY-AND-ROLE-FIELDS-AUDIT-1 found `cases.clientRole` present-compatible and active in case DTOs, create/update flows, timeline payloads, and anonymization targeting. Current evidence shows general authentication on generic case routes, but no dedicated case manager/role policy on the inspected generic update path before changing `clientRole`. The field can reveal party/legal-side context and affect anonymization semantics. | Define allowed values and product meaning, then prove case-level authorization for reads/writes before any narrow internal `KEEP`. Do not treat as Client Portal, external visibility, production apply, CP-SCHEMA-1, or DB migration replay. |
...
- Whether client identity fields and `cases.clientRole` should move beyond `KEEP-BUT-HARDEN candidate` after dedicated authorization, DTO, privacy, and product-semantics decisions. `clients.color` is separately decided as narrow internal `KEEP` only.
