...
- Anonymous documents.
- Rehydration fields.
- Client identity fields (`KEEP-BUT-HARDEN candidate` after audit).
- Case client role (`hardened internal KEEP candidate`; semantics decided as internal matter-party metadata and route authorization hardened).
- Comparison snapshot.
- Workspace text.
...
| Client identity fields | `KEEP-BUT-HARDEN candidate` | CLIENT-IDENTITY-AND-ROLE-FIELDS-AUDIT-1 created `docs/client-identity-role-fields-audit.md`. Production metadata compare found `clients.taxNumber`, `clients.companyRegistrationNumber`, and `clients.authorizedRepresentative` present-compatible, and active internal client routes read/write them. Current evidence shows general authentication on client list/detail/create/update routes, but no dedicated client ownership, need-to-know, assigned-lawyer, case-membership, or role-scoped authorization policy for these identity fields. `clients.color` KEEP remains separate and does not include identity fields. | Keep internal-only as a candidate. Required before narrow internal `KEEP`: client/case-level or role-scoped authorization decision, DTO/search boundary tests, no Client Portal reuse, no external mapper exposure, privacy/GDPR review, and targeted unauthenticated/unauthorized/authorized tests. |
| Case client role | `hardened internal KEEP candidate` | Production metadata found `cases.clientRole` present-compatible. CASES-CLIENT-ROLE-SEMANTICS-DECISION-1 selected internal matter-party metadata semantics. CASES-CLIENT-ROLE-INTERNAL-HARDEN-1 (`e2a943a`) added `requireCaseManageAccess`; guards detail, summary, and workflow reads with case-level read access; guards generic patch/update with case-manager access; removes `clientRole` from the broad case list DTO; and keeps create under the existing authenticated create-case rules. Full backend validation passed 15 suites / 161 tests. `cases.clientRole` is not a Client Portal display field, tenant boundary, authorization primitive, external label, enum contract, or public API contract. | Candidate only, not broad `KEEP`. Required before full narrow internal `KEEP`: explicit human decision, confirm no unreviewed broad list/search/export/report route exposes `clientRole`, confirm no Client Portal/external mapper exposure, preserve focused tests, and keep production apply / CP-SCHEMA-1 blocked until separately resolved. Do not treat as Client Portal, external visibility, production apply, CP-SCHEMA-1, schema migration, DB migration replay, or authorization primitive. |
...
- Whether client identity fields should move beyond `KEEP-BUT-HARDEN candidate` after dedicated authorization, DTO, and privacy decisions; and whether `cases.clientRole` should move beyond `hardened internal KEEP candidate` only after a separate keep-decision closeout. `clients.color` is separately decided as narrow internal `KEEP` only.
