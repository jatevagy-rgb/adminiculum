...
| Case client role | `hardened internal KEEP candidate` | CASES-CLIENT-ROLE-SEMANTICS-DECISION-1 created `docs/cases-client-role-semantics-decision.md` and selected Option A: internal matter-party metadata. CASES-CLIENT-ROLE-INTERNAL-HARDEN-1 adds case-level read authorization before detail, summary, and workflow responses can return `cases.clientRole`; removes `clientRole` from the broad case list DTO; and requires case-manager access for generic case patch/update. Create remains protected by the existing authenticated create-case rules. `cases.clientRole` is not a Client Portal display field, tenant boundary, authorization primitive, external label, or enum contract. | Candidate only, not broad `KEEP`. Required before narrow internal `KEEP`: dedicated keep-decision closeout after deployment/smoke if applicable, continued no Client Portal/external mapper exposure, and no production migration replay. Do not treat as Client Portal, external visibility, production apply, CP-SCHEMA-1, schema migration, DB migration replay, or authorization primitive. |
