...
- Anonymous documents.
- Rehydration fields.
- Client identity fields (`hardened internal KEEP candidate` after CLIENT-IDENTITY-FIELDS-HARDEN-1).
...
| Client identity fields | `hardened internal KEEP candidate` | Production metadata is present-compatible. CLIENT-IDENTITY-FIELDS-HARDEN-1 (`8cea64c`) adds internal authorization/exposure hardening for client identity/contact fields: broad `GET /api/v1/clients` is scoped for non-privileged users to clients linked to cases where they are assigned lawyer, creator, or collaborator; `GET /api/v1/clients/:clientId` requires `ADMIN` / `PARTNER` or related-case access; `POST /api/v1/clients` and `PATCH /api/v1/clients/:clientId` require `ADMIN` / `PARTNER`; Client Portal remains disabled/quarantined. Targeted `clientIdentityFieldsAuthz` tests passed 11/11, and full backend tests passed 16 suites / 172 tests. `clients.color` KEEP remains separate and does not include identity fields. | Candidate only, not broad `KEEP`. Required before narrow internal `KEEP`: separate human decision, preserved route tests, confirmation that no unreviewed client search/dashboard/export/report or house-style/client-portal route exposes identity fields, no external mapper exposure, no Client Portal reuse, privacy/GDPR review, and no production apply, CP-SCHEMA-1, schema migration, or DB migration replay. |
...
- Whether client identity fields should move beyond `hardened internal KEEP candidate` after a separate human keep decision, DTO/search/export boundary review, and privacy decision. `cases.clientRole` is separately decided as `KEEP — narrow internal baseline` only for hardened internal matter-party metadata behavior, and `clients.color` is separately decided as narrow internal `KEEP` only.
