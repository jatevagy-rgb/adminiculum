...
| Client identity fields | `hardened internal KEEP candidate` | CLIENT-IDENTITY-FIELDS-HARDEN-1 adds internal authorization/exposure hardening for client identity/contact fields. Broad `GET /api/v1/clients` is scoped for non-privileged users to clients linked to cases where they are assigned lawyer, creator, or collaborator; `GET /api/v1/clients/:clientId` requires `ADMIN` / `PARTNER` or related-case access; `POST /api/v1/clients` and `PATCH /api/v1/clients/:clientId` require `ADMIN` / `PARTNER`; Client Portal remains disabled/quarantined. Targeted `clientIdentityFieldsAuthz` tests cover unauthenticated, unauthorized, related-case, admin/partner, list-scoping, and portal-gate paths. `clients.color` KEEP remains separate and does not include identity fields. | Candidate only, not broad `KEEP`. Required before narrow internal `KEEP`: separate human decision, preserved route tests, confirmation that no unreviewed search/dashboard/export/report or house-style/client-portal route exposes identity fields, no external mapper exposure, no Client Portal reuse, privacy/GDPR review, and no production apply, CP-SCHEMA-1, schema migration, or DB migration replay. |
