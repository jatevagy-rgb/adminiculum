...
Final classification target: `workload_records_authorization_hardened_no_db_change_no_migration_no_azure`
...
| Workload tracking / `workload_records` | `hardened internal KEEP candidate` | WORKLOAD-RECORDS-INTERNAL-EXPOSURE-AUDIT-1 created `docs/workload-records-exposure-audit.md`. Production metadata compare found `workload_records` present-compatible and the repo has active internal workgroup/workload usage. WORKLOAD-RECORDS-HARDEN-1 added conservative `ADMIN` / `PARTNER` authorization for workgroup/workload read/write routes and targeted tests proving unauthenticated and ordinary authenticated users are rejected before workload DB access. Public OpenAPI metadata still quarantines workgroup/workload paths. Client Portal remains disabled/quarantined. | Do not promote to full `KEEP` without a separate human decision. Keep limited to hardened internal `ADMIN` / `PARTNER` routes. Do not treat as Client Portal, external visibility, CP-SCHEMA-1, production apply, self-scoped lawyer/team access, or DB migration replay. Future non-admin scoped access requires a separate membership/ownership model and tests. |
