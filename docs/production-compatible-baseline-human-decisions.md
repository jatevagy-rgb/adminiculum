...
- Whether client identity fields should move beyond `KEEP-BUT-HARDEN candidate` after dedicated authorization, DTO, and privacy decisions. `cases.clientRole` is separately decided as `KEEP — narrow internal baseline` only for hardened internal matter-party metadata behavior, and `clients.color` is separately decided as narrow internal `KEEP` only.
