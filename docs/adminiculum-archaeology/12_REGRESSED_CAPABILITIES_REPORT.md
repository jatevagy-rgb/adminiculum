...
| "Version-history" (immutable DocumentVersion) — NOT a regression | DocumentVersion backend complete; case document UI loads/upload/downloads/promotes/render history | **None — it was NOT removed.** What was removed is the legacy editor working-copy autosave/track-changes save path | editor hardening (`DOCUMENT-EDITOR-PRO-*`) | guards removed editor autosave/track-changes only; immutable DocumentVersion presentation survived | **No recovery needed** — intentionally retained capability, not a loss |
