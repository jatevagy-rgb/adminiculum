...
4. **Version-history UI was removed by design** in the editor (guards strip autosave/track-changes), but the backend `DocumentVersion` list/get/promote-current API is complete, and the case documents page already receives a `versions` prop (`DocumentReviewWorkflowPanel({ documentId, selectedVersionId, versions })`). The version-history *presentation* is recoverable — **no backend change needed.**
