...
**A large majority of the future roadmap is NOT greenfield.** The client/portal/identity core, the task submission/review core, the document review/compare/anonymize core, the work-package/workflow core, and the communication core are all already built and mounted at canonical. What is actually missing is a handful of **connections** (case→workpackage on modern paths, case→portal for internal intake, DOCX text-diff, version-history UI) and a small set of **true greenfield** items (persisted thread/unread/reply, outgoing mail, case-level reviewer, billing). This materially shortens the roadmap.
