...
- CASES-CLIENT-ROLE-INTERNAL-HARDEN-1 (`e2a943a`) moved only `cases.clientRole` from `KEEP-BUT-HARDEN candidate` to `hardened internal KEEP candidate`; CASES-CLIENT-ROLE-HARDENING-CLOSEOUT-1 documents that evidence but does not move it to broad `KEEP`.
