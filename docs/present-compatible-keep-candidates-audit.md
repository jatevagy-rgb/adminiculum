...
- CLIENT-IDENTITY-FIELDS-HARDEN-1 (`8cea64c`) moved only client identity fields from `KEEP-BUT-HARDEN candidate` to `hardened internal KEEP candidate`; CLIENT-IDENTITY-FIELDS-HARDENING-CLOSEOUT-1 documents that evidence and prepares, but does not finalize, a possible future narrow internal `KEEP` decision.
