...
| Client identity fields: `clients.taxNumber`, `clients.companyRegistrationNumber`, `clients.authorizedRepresentative` | present-compatible | Prisma model includes fields; clients list enriches with identity fields; create/update routes write them; frontend clients pages/search display/edit them. | `GET /api/v1/clients`, `GET /api/v1/clients/:clientId`, `POST /api/v1/clients`, `PATCH /api/v1/clients/:clientId`; fields appear in internal frontend client/search surfaces. | CLIENT-IDENTITY-FIELDS-HARDEN-1 (`8cea64c`) scopes non-privileged list/detail reads to related-case access and limits create/update to `ADMIN` / `PARTNER`; targeted authz/portal-gate tests exist. | Legal/business identity data; appropriate for current hardened internal client management only, not automatically client-visible and not a Client Portal identity model. | KEEP — narrow internal baseline | Preserve related-case read scoping, `ADMIN` / `PARTNER` management guard, and tests; future search/export/report, external mapper, Client Portal, or broader ownership model requires separate review. | `CLIENT-IDENTITY-FIELDS-INTERNAL-KEEP-DECISION-1` |
...
- CASES-CLIENT-ROLE-INTERNAL-HARDEN-1 (`e2a943a`) moved only `cases.clientRole` from `KEEP-BUT-HARDEN candidate` to `hardened internal KEEP candidate`; CASES-CLIENT-ROLE-HARDENING-CLOSEOUT-1 (`05791ef`) documented that evidence; CASES-CLIENT-ROLE-INTERNAL-KEEP-DECISION-1 moves only the hardened internal matter-party metadata behavior to `KEEP — narrow internal baseline`, not broad/external/Client Portal `KEEP`.
- CLIENT-IDENTITY-FIELDS-HARDEN-1 (`8cea64c`) moved only client identity fields from `KEEP-BUT-HARDEN candidate` to `hardened internal KEEP candidate`; CLIENT-IDENTITY-FIELDS-HARDENING-CLOSEOUT-1 documents that evidence and limitations.
- CLIENT-IDENTITY-FIELDS-INTERNAL-KEEP-DECISION-1 moves only the hardened internal client identity route behavior from `hardened internal KEEP candidate` to `KEEP — narrow internal baseline`, not broad/external/Client Portal `KEEP`.
- Present-compatible internal `KEEP` baseline now includes `clients.color`, `case_collaborators`, `workload_records`, `cases.clientRole`, and client identity fields, each only within its documented narrow internal boundary.
- DOCUMENTS-WORKSPACE-TEXT-PRIVACY-AUDIT-1 created `docs/documents-workspace-text-privacy-audit.md` and confirms `documents.workspaceText` remains the unresolved `SECURITY/PRIVACY BLOCKED` present-compatible item.
- This does not move Client Portal, external client visibility, or `documents.workspaceText` out of their prior lanes.
...
4. `CLIENT-FIELDS-INTERNAL-BASELINE-DECISION-1`
   - Completed through separate narrow internal decisions for `clients.color`, `cases.clientRole`, and client identity fields.
   - These decisions do not enable Client Portal or external visibility.
   - No runtime change.
