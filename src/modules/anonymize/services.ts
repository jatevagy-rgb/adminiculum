...
export async function anonymizeDocument(params: {
  ...
  try {
    ...
    if (clientData) {
      if (clientData.taxId) content += `Adószám: ${clientData.taxId}\n`;
      if (clientData.bankAccount) content += `Bankszámla: ${clientData.bankAccount}\n`;
      if (clientData.address) content += `Cím: ${clientData.address}\n`;
