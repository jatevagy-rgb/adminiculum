...
export class HungarianNumberToWords {
  private static readonly unitWords: string[] = [
  ...
  private static readonly tenWords: string[] = [
    '', '', 'húsz', 'harminc', 'negyven', 'ötven', 'hatvan', 'hetven', 'nyolcvan', 'kilencven'
  ];

  private static readonly hundredWords: string[] = [
  ...
  static convert(number: number | string): string {
    if (!number) return 'nulla';
    
    const num = typeof number === 'string' ? parseFloat(number) : number;
    
    if (isNaN(num)) return 'nulla';
    if (num < 0) return 'mínusz ' + this.convert(-num);
  ...
  static toForint(amount: number | string): string {
    ...
    if (isNaN(num)) return 'nulla forint';
