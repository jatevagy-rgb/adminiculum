 ...
 * Seeds the default adásvételi contract template
 */
...
async function main() {
  ...
  const existingTemplate = await prisma.contractTemplate.findFirst({
    where: {
      category: TemplateCategory.ADASVETEL,
      isDefault: true
    }
  });

  if (existingTemplate) {
    console.log('Default adásvételi template already exists, skipping...');
    return;
  }

  // Create default adásvételi template
  const template = await prisma.contractTemplate.create({
    data: {
      name: 'Ingatlan adásvételi szerződés',
      description: 'Standard ingatlan adásvételi szerződés sablon',
      category: TemplateCategory.ADASVETEL,
      templatePath: path.join(process.cwd(), 'templates', 'adasveteli_sablon.docx'),
      originalFileName: 'adasveteli_sablon.docx',
      variables: ADASVETEL_VARIABLES as any,
      isActive: true,
      isDefault: true
    }
  });
